...
class NodeResolver {
  ...
  async resolve({ filename, parent, specifierType, env, sourcePath, loc }) {
    let ctx = {
    ...
    try {
      ...
      if (module.moduleDir) {
        ...
        resolved = await this.loadRelative(
          ...
          extensions,
          env,
          parent ? _path().default.dirname(parent) : this.projectRoot,
          ctx
        );
      }
  ...
  async resolveModule({ filename, parent, env, ctx, sourcePath }) {
    ...
    if (parent) {
      let res = await this.resolveFilename(
        filename,
        _path().default.dirname(sourceFile),
        ctx.specifierType
      );
      ...
      if (packageManager) {
        ...
        (_this$logger = this.logger) === null || _this$logger === void 0
          ...
          : _this$logger.warn({
              message: (0,
              _diagnostic()
                .md)`Auto installing polyfill for Node builtin module "${specifier}"...`,
              codeFrames: [
                {
                  filePath:
                    (_ctx$loc$filePath =
                      (_ctx$loc = ctx.loc) === null || _ctx$loc === void 0
                        ? void 0
                        : _ctx$loc.filePath) !== null &&
                    _ctx$loc$filePath !== void 0
                      ? _ctx$loc$filePath
                      : sourceFile,
        ...
        throw new (_diagnostic().default)({
          diagnostic: {
            message: (0,
            _diagnostic()
              .md)`Node builtin polyfill "${packageName}" is not installed, but auto install is disabled.`,
            codeFrames: [
              {
                filePath:
                  (_ctx$loc$filePath2 =
                    (_ctx$loc2 = ctx.loc) === null || _ctx$loc2 === void 0
                      ? void 0
                      : _ctx$loc2.filePath) !== null &&
                  _ctx$loc$filePath2 !== void 0
                    ? _ctx$loc$filePath2
                    : sourceFile,
                codeHighlights: ctx.loc
                  ? [
    ...
    if (resolved === undefined && process.versions.pnp != null && parent) {
      try {
        ...
        let res = pnp.resolveToUnqualified(
          moduleName + // retain slash in `require('assert/')` to force loading builtin from npm
    ...
    if (resolved === undefined) {
      ...
      let alternativeModules = await (0, _utils().findAlternativeNodeModules)(
        this.fs,
        moduleName,
        _path().default.dirname(sourceFile)
      );
      ...
      if (alternativeModules.length) {
        ...
        throw new (_diagnostic().default)({
          diagnostic: {
            message: (0, _diagnostic().md)`Cannot find module ${(0,
            _nullthrows().default)(
              (_resolved2 = resolved) === null || _resolved2 === void 0
                ? void 0
                : _resolved2.moduleName
            )}`,
  ...
  async checkExcludedDependency(sourceFile, name, ctx) {
    ...
    if (
      !(
        ...
        _pkg$dependencies[moduleName]
      ) &&
      !(
        (_pkg$peerDependencies = pkg.peerDependencies) !== null &&
        _pkg$peerDependencies !== void 0 &&
        _pkg$peerDependencies[moduleName]
      ) &&
      !(
        (_pkg$engines = pkg.engines) !== null &&
        _pkg$engines !== void 0 &&
        _pkg$engines[moduleName]
      )
    ) {
      ...
      throw new (_diagnostic().default)({
        diagnostic: {
          message: (0,
          _diagnostic()
            .md)`External dependency "${moduleName}" is not declared in package.json.`,
          codeFrames: [
            {
              ...
              codeHighlights: pkg.dependencies
                ? (0, _diagnostic().generateJSONCodeHighlights)(pkgContent, [
                    {
                      key: `/dependencies`,
                      type: 'key',
                    },
  ...
  async resolveFilename(filename, dir, specifierType) {
    ...
    switch (filename[0]) {
      ...
      case '~': {
        ...
        while (
          dir !== this.projectRoot &&
          _path().default.basename(_path().default.dirname(dir)) !==
            'node_modules' &&
          (insideNodeModules ||
            !(await this.fs.exists(_path().default.join(dir, 'package.json'))))
        ) {
    ...
    if (specifierType === 'url' || specifierType === 'esm') {
      ...
      url =
        (_url = url) !== null && _url !== void 0
          ? _url
          : new URL(filename, `file:${dir}/index`);
      let filePath;
  ...
  async loadRelative(filename, extensions, env, parentdir, ctx) {
    ...
    if (!resolvedFile) {
      ...
      let relativeFileSpecifier = (0, _utils().relativePath)(
        parentdir,
        filename
      );
      let potentialFiles = await (0, _utils().findAlternativeFiles)(
        this.fs,
      ...
      throw new (_diagnostic().default)({
        diagnostic: {
          message: (0,
          _diagnostic()
            .md)`Cannot load file goodbye '${relativeFileSpecifier}' in '${(0,
          _utils().relativePath)(this.projectRoot, parentdir)}'.`,
  ...
  findNodeModulePath(filename, sourceFile, ctx) {
    ...
    if (moduleDir) {
      return {
        ...
        moduleDir,
        filePath: subPath
          ? _path().default.join(moduleDir, subPath)
          : moduleDir,
      };
    }
  ...
  async loadDirectory({ dir, extensions, env, ctx, pkg }) {
    var _pkg;
    ...
    try {
      ...
      if (failedEntry && pkg) {
        ...
        let fileSpecifier = (0, _utils().relativePath)(
          dir,
          failedEntry.filename
        );
        let alternatives = await (0, _utils().findAlternativeFiles)(
          this.fs,
          fileSpecifier,
          pkg.pkgdir,
          this.projectRoot
        ...
        throw new (_diagnostic().default)({
          diagnostic: {
            message: (0,
            _diagnostic()
              .md)`Could not load '${fileSpecifier}' from module '${pkg.name}' found in package.json#${failedEntry.field}`,
            codeFrames: [
              {
                filePath: pkg.pkgfile,
                language: 'json',
                code: pkgContent,
                codeHighlights: (0, _diagnostic().generateJSONCodeHighlights)(
                  pkgContent,
                  [
                    {
                      ...
                      type: 'value',
                      message: (0,
                      _diagnostic().md)`'${fileSpecifier}' does not exist${
    ...
    return this.loadAsFile({
      ...
      env,
      pkg:
        (_pkg = pkg) !== null && _pkg !== void 0
          ? _pkg
          : await this.findPackage(_path().default.join(dir, 'index'), ctx),
      ctx,
    });
  }
  ...
  getPackageEntries(pkg, env) {
    return this.mainFields
      ...
      .map(entry => {
        (0, _assert().default)(
          entry != null && typeof entry.filename === 'string'
  ...
  async loadAsFile({ file, extensions, env, pkg, ctx, debug }) {
  ...
  async getAlias(filename, pkg, aliases) {
    ...
    if (_path().default.isAbsolute(filename)) {
      ...
      alias = this.lookupAlias(
        aliases,
        (0, _utils().normalizeSeparators)(filename)
      );
      ...
      if (alias.global) {
        if (typeof alias.global !== 'string' || alias.global.length === 0) {
          throw new (_diagnostic().default)({
            diagnostic: {
              message: (0,
              _diagnostic().md)`The global alias for ${filename} is invalid.`,
  ...
  async findPackage(sourceFile, ctx) {
    ...
    let res = await (0, _utils().loadConfig)(
      this.fs,
      sourceFile,
      ['package.json'],
      this.projectRoot, // By default, loadConfig uses JSON5. Use normal JSON for package.json files
  ...
  async loadAlias(filename, sourceFile, env, ctx) {
    ...
    if (!this.rootPackage) {
      this.rootPackage = await this.findPackage(
        _path().default.join(this.projectRoot, 'index'),
        ctx
      );
  ...
  getModuleParts(name) {
    ...
    if (splitOn < 0) {
      ...
      return [
        (0, _utils().normalizeSeparators)(name.substring(0, splitOn)),
